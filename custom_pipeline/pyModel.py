...
class ControlledModel(nn.Module):
    ...
    def forward(self, x, lengths=None, extract_intermediates=False):
        ...
        # -------------------------------------------------------------
        # CUSTOM CUDA KERNEL CALL: pad_truncate
        # -------------------------------------------------------------
        if lengths is not None and custom_cuda_ops is not None:
            # We use the custom C++ PyTorch extension to pad/truncate on the GPU
            ...
            # Ensure it is long for PyTorch Embedding layer
            x = x.to(torch.long)
...
def run_training():
    ...
    data_path = "../scripts/yelp_tokenized.npz"
